...
import { nodeTypes } from './NodeTypesChakra'
import { edgeTypes } from './EdgeTypes'
import { NodeConfigurationPanel } from './NodeConfigPanelChakra'
import { SourceConnectionsSidebar } from './SourceConnectionsSidebar'
...
import { EnhancedFilterConfigPanel } from './EnhancedFilterConfigPanel'
import { JoinConfigPanel } from './JoinConfigPanel'
import { ProjectionConfigPanel } from './ProjectionConfigPanel'
import { DestinationConfigPanel } from './DestinationConfigPanel'
import { CalculatedColumnConfigPanel } from './CalculatedColumnConfigPanel'
import { AggregatesConfigPanel } from './AggregatesConfigPanel'
import { ComputeNodeConfigPanel } from './ComputeNodeConfigPanel'
import { TableDataPanel } from './TableDataPanel'
import { ColumnDefinitionsMenu } from './ColumnDefinitionsMenu'
...
interface DataFlowCanvasChakraProps {
  canvasId?: number
  initialNodes?: Node[]
  initialEdges?: Edge[]
  sourceId?: number
...
export const EnhancedDataFlowCanvas: React.FC<DataFlowCanvasChakraProps> = ({
  canvasId,
  initialNodes = [],
  initialEdges = [],
  sourceId,
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        const res = await migrationApi.getAll()
        ...
        const statusRes = await migrationApi.getStatus(running.id)
        const data = statusRes?.data as { node_progress?: Array<{ node_id: string; status: string }>; current_level?: number; total_levels?: number; level_status?: string }
        if (data && !cancelled) {
    ...
    const poll = async () => {
      try {
        ...
        if (status === 'running' || status === 'pending') {
          try {
            const statusRes = await migrationApi.getStatus(job.id)
            const data = statusRes?.data as { node_progress?: Array<{ node_id: string; status: string }>; status?: string; current_level?: number; total_levels?: number; level_status?: string }
  ...
  const handleSaveCanvas = useCallback(async (id: number | null, name: string) => {
    ...
    try {
      ...
      if (id) {
        ...
        const response = await canvasApi.create(createPayload)
        console.log('[CANVAS CREATE] Response:', response.data)

        if (response.data && response.data.id) {
          const newCanvasId = response.data.id
          const newCanvasName = response.data.name || name.trim()
          ...
          throw new Error('Canvas creation failed: No ID returned from server')
        }
      }
    } catch (error: any) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (canvasId) {
      try {
        ...
        const currentName = canvasResponse.data.name || ''
  ...
  const handleValidate = useCallback(async (requireDestination: boolean = true) => {
    ...
    try {
      ...
      const res = await metadataApi.validatePipeline(payload.nodes, payload.edges, canvasId)
      const backendErrors = (res?.data?.errors as string[]) || []
  ...
  const handleExecute = useCallback(async (flowNodeIds?: string[]) => {
    ...
    try {
      ...
      async function catchUpStatusForJob(jid: string) {
        try {
          const res = await migrationApi.getAll()
          const jobs = (res?.data ?? []) as Array<{ id: number; job_id: string; status: string }>
          const job = jobs.find((j) => j.job_id === jid)
          if (!job || !['running', 'pending', 'completed', 'failed'].includes(String(job.status).toLowerCase())) return
          const statusRes = await migrationApi.getStatus(job.id)
          const data = statusRes?.data as { node_progress?: Array<{ node_id: string; status: string }>; current_level?: number; total_levels?: number; level_status?: string; status?: string; progress?: number; current_step?: string }
          if (!data) return
          if (data.node_progress?.length) {
            const batch: Record<string, 'idle' | 'running' | 'success' | 'error'> = {}
            data.node_progress.forEach((np: { node_id: string; status: string }) => {
