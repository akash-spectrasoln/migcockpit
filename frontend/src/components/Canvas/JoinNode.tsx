/**
 * Join Node Component
 * Visual representation of a join transformation node
 */
...
export const JoinNode: React.FC<NodeProps<JoinNodeData>> = ({ data, selected }) => {
  ...
  const joinType = data.config?.joinType || 'INNER'
  const conditionCount = data.config?.conditions?.length || 0
  const displayName = data.business_name || data.label || 'Join'
    ...
    >
      ...
      <VStack align="flex-start" spacing={1}>
        <HStack spacing={1.5} w="100%">
          <Icon as={GitMerge} color={`${color}.500`} w={4} h={4} />
        ...
        <Badge colorScheme={joinTypeColors[joinType] || 'gray'} size="sm" fontSize="2xs" px={1.5} py={0.5}>
          {joinType}
        </Badge>

        {conditionCount > 0 && (
          <Badge colorScheme={color} size="sm" fontSize="2xs" px={1.5} py={0.5}>
            {conditionCount} key{conditionCount !== 1 ? 's' : ''}
          </Badge>
        )}
