...
export const ProjectionConfigPanel: React.FC<ProjectionConfigPanelProps> = ({
  ...
  useEffect(() => {
    ...
    // Load columns from compiledGraph (O(1), no backend API calls needed)
    if (node && compiledGraph) {
      const compiledNode = compiledGraph.nodes[node.id]
      if (compiledNode && compiledNode.inputSchema) {
        // Convert internal ColumnSchema to UI ColumnMetadata format
        const newCols: ColumnMetadata[] = compiledNode.inputSchema.map(col => ({
          name: col.name,
          business_name: col.name,
          technical_name: col.column || col.name,
        ...
        // Only update if there's a difference to prevent infinite render loops
        setAvailableColumns(prev => {
          if (prev.length === newCols.length && prev.every((p, i) => p.name === newCols[i].name && p.datatype === newCols[i].datatype)) {
            return prev
          }
          return newCols
        })
  ...
  }, [node?.id, node?.data?.config?.output_columns, node?.data?.config?.includedColumns, node?.data?.config?.excludedColumns, node?.data?.config?.calculatedColumns, compiledGraph])
