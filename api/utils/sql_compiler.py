...
from api.views.misc import build_group_by_sql
...
class SQLCompiler:
    ...
    def compile(self) -> Tuple[str, List[Any], Dict]:
        ...
        initial_params_count = len(self.params)
    ...
    def _build_join_cte(self, node: Dict[str, Any]) -> Tuple[str, Dict]:
        ...
        if output_columns:
            ...
            conflicts = []  # Track conflicts for resolution
            ...
            for final_name, (table_alias, col_clean, src) in output_names_used.items():
            ...
            if tech:
                ...
                if col_name in left_cols or col_name in right_cols:
                    ...
                    if not origin:
                        ...
                        in_right = col_name in right_cols
    ...
    def _build_projection_cte(self, node: Dict[str, Any]) -> Tuple[str, Dict]:
        ...
        if selected_mode == 'EXCLUDE' or config.get('excludeMode'):
            ...
            if selected_columns:
                ...
                if mapped_columns:
                    ...
                    logger.warning(f"Projection: No valid columns after mapping, using all available columns")
        ...
        if not validated_base_columns:
            logger.warning(f"Projection: No valid columns after validation, using all available columns")
    ...
    def _get_source_config(self, source_id: int) -> Dict[str, Any]:
        ...
        conn = psycopg2.connect(
            ...
            database=self.customer.cust_db
        )
