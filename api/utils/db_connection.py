...
def get_customer_from_request(request) -> Optional[Any]:
    ...
    """
    from api.models import Customer, Canvas
    ...
    if hasattr(request, 'user') and hasattr(request.user, 'cust_id_id') and request.user.cust_id_id:
        ...
        logger.debug(f"[DB_CONFIG] User has no cust_id_id")
    ...
    if canvas_id:
        ...
        logger.debug(f"[DB_CONFIG] No canvas_id found in request")
    
    logger.warning(f"[DB_CONFIG] ✗ Could not determine customer from request")
    return None
