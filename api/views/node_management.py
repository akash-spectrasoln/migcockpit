...
class NodeInsertionView(APIView):
    ...
    def post(self, request):
        ...
        """
        try:
            logger.info(f"[NODE INSERT] Received insert node request")
            ...
            if frontend_nodes is not None and frontend_edges is not None:
                logger.info(f"[NODE INSERT] Using nodes/edges from frontend request")
                ...
                logger.info(f"[NODE INSERT] Loading from canvas.configuration")
                ...
                if isinstance(config, dict):
                    ...
                    logger.warning(f"[NODE INSERT] Configuration is not a dict, attempting to parse")
                    nodes = []
                    edges = []
                ...
                if len(nodes) == 0 or len(edges) == 0:
                    logger.warning(f"[NODE INSERT] Configuration appears empty, attempting to load from CanvasNode/CanvasEdge models")
                    try:
            ...
            logger.info(f"[NODE INSERT] Step 3: Creating new edges")
            ...
            if not edge_a_to_x or not edge_x_to_b:
                logger.error(f"VALIDATION FAILED: Missing edges after insertion!")
            ...
            logger.info(f"VALIDATION PASSED: Graph structure is correct after insertion")
            ...
            return Response(
                {"error": f"Failed to insert node: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class PipelineRecomputeView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.error(traceback.format_exc())
            return Response(
                {"error": f"Failed to find recompute path: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
