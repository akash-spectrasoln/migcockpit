...
from api.models import SourceDB, SourceForm, Customer, Country
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                # If customer is None or cust_db is None, use the default database
                # Determine which database to use
                # customer is guaranteed to exist after ensure_user_has_customer
                target_database = customer.cust_db
                ...
                name_col = 'source_name' if 'source_name' in final_columns else 'src_name'
                config_col = 'source_config' if 'source_config' in final_columns else 'src_config'
                ...
                print(f"Error in SourceConnectionCreateView: {str(e)}")
                print(f"Traceback: {error_traceback}")
                return Response(
                    {"error": f"Failed to store source details in customer database: {str(e)}", "traceback": error_traceback},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class CustomerSourcesView(APIView):
    ...
    def get(self, request,):
        try:
            ...
            # Use source_name if exists, otherwise fallback to src_name
...
class SourceAttributesView(APIView):
    ...
    def get(self, request, source_type=None):
        ...
        """
        try:
            from api.models import SourceModel, SourceAttribute
            ...
            return Response(
                {"error": f"Failed to fetch source attributes: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class SourceConnectionCreateWithValidationView(APIView):
    ...
    def post(self, request):
        ...
        """
        try:
            from api.models import SourceModel, SourceConfig
            import requests
            ...
            traceback.print_exc()
            return Response(
                {"error": f"Failed to create source connection: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def get(self, request, source_id):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {
                        "error": f"FastAPI service error: {str(e)}",
                        "service_url": EXTRACTION_SERVICE_URL
                    },
                    status=status.HTTP_503_SERVICE_UNAVAILABLE
                )
    ...
    def get(self, request, source_id):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"error": f"Failed to fetch table data: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    ...
    def get(self, request, source_id):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"error": f"Failed to fetch columns: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
class SourceTableSelectionView(APIView):
    ...
    def post(self, request, source_id):
        ...
        try:
            ...
            logger.error(traceback.format_exc())
            return Response(
                {"error": f"Failed to save selected tables: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def get(self, request, source_id):
        ...
        try:
            ...
            logger.error(traceback.format_exc())
            return Response(
                {"error": f"Failed to fetch selected tables: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete(self, request, source_id):
        ...
        try:
            ...
            logger.error(traceback.format_exc())
            return Response(
                {"error": f"Failed to unselect tables: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class SourceLiveSchemaView(APIView):
    ...
    def get(self, request, source_id, table_name):
        ...
        try:
            ...
            return Response({"error": f"FastAPI service error: {str(exc)}"}, status=status.HTTP_502_BAD_GATEWAY)
        except Exception as exc:
            import traceback
            logger.error(f"[SourceLiveSchemaView] {exc}\n{traceback.format_exc()}")
            return Response({"error": f"Failed to fetch live schema: {str(exc)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
