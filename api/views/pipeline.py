...
from api.utils.helpers import ensure_user_has_customer, decrypt_source_data
from api.utils.graph_utils import strip_orphaned_edges, validate_dag
from api.utils.filters import parse_filter_from_canvas, build_sql_where_clause
from api.utils.calculated_column_evaluator import evaluate_calculated_expression
from api.views.misc import (
    apply_auto_group_by_rule,
    build_group_by_sql,
    validate_aggregate_configuration
)
...
class FilterExecutionView(APIView):
    ...
    def post(self, request, source_id):
        ...
        try:
            ...
            if len(conditions) == 0:
                logger.error(f"FilterExecutionView: conditions array is empty")
                return Response(
            ...
            try:
                ...
                if source_schema:
                    ...
                    logger.warning(f"FilterExecutionView: No source schema available")
                    result_columns = [
                ...
                return Response(
                    {"error": f"Failed to execute filter: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
    ...
    def post(self, request, source_id):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"error": f"Failed to execute join: {str(e)}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
...
class PipelineQueryExecutionView(APIView):
    ...
    def _evaluate_calculated_column(self, expression: str, row: dict, available_columns: list, debug_steps=None):
        ...
        try:
            ...
            logger.error(f"[Calculated Column] Error evaluating expression '{expression}': {str(e)}")
            debug_steps.append({
                'stage': 'ERROR',
                'input': expression,
                'output': None,
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(f"=" * 60)
            logger.info(f"PipelineQueryExecutionView: NEW REQUEST")
            ...
            if preview_mode in ("output", "input"):
                ...
                if target_node_type == 'compute':
                    ...
                    try:
                        ...
                        # Provide both 'df' and '_input_df' for flexibility
                        local_vars = {
                            'df': input_df, 
                        ...
                        output_df = local_vars.get('_output_df') if local_vars.get('_output_df') is not None else local_vars.get('output_df')
                        ...
                        return Response({"error": f"Compute execution failed: {str(e)}"}, status=500)
                ...
                try:
                    ...
                    if cached_data:
                        ...
                        if current_calculated_columns and len(current_calculated_columns) > 0:
                            logger.info(f"Cache hit but calculated columns detected - will reprocess calculated columns on cached base data")
            ...
            if target_node_type == 'join':
                ...
                if parent_node and parent_node.get('data', {}).get('type') == 'join':
                    ...
                    from api.filters import parse_filter_from_canvas
            ...
            if join_node is not None:
                ...
                logger.info(f"PipelineQueryExecutionView: executing join pipeline")
                ...
                if not left_edge and not right_edge:
                    ...
                    if len(input_edges) >= 2:
                        logger.warning(f"[Join Execution] No explicit handles found, using order-based assignment (first=left, second=right)")
                        ...
                        logger.error(f"[Join Execution] Only one edge found without explicit handles - cannot determine left vs right")
                        return Response(
                    ...
                    logger.warning(f"[Join Execution] Right handle found but not left, searching for left edge")
                    left_edge = next((e for e in input_edges if e.get('source') != right_edge.get('source')), None)
                    if not left_edge:
                        logger.error(f"[Join Execution] Could not find a different edge for left handle")
                        return Response(
                    ...
                    logger.warning(f"[Join Execution] Left handle found but not right, searching for right edge")
                    right_edge = next((e for e in input_edges if e.get('source') != left_edge.get('source')), None)
                    if not right_edge:
                        logger.error(f"[Join Execution] Could not find a different edge for right handle")
                        return Response(
                ...
                logger.info(f"[Join Execution] Resolved LEFT input:")
                ...
                logger.info(f"[Join Execution] Resolved RIGHT input:")
                ...
                if (left_node.get('id') != right_node.get('id') and 
                    ...
                    logger.error(f"[Join Execution] VALIDATION FAILED: Distinct input nodes resolved to same source table")
                ...
                try:
                    ...
                    if (left_source_node_id == right_source_node_id and 
                        ...
                        logger.info(f"[Join Execution] This is NOT a self-join - using normal join logic")
                    ...
                    if is_self_join:
                        ...
                        logger.info(f"[Join Execution] Using __L__ and __R__ prefixes for column identification")
                    ...
                    logger.info(f"[Join Execution] Calling FastAPI /metadata/join endpoint")
                    ...
                    return Response(
                        {
                            ...
                            "details": f"Join execution failed at FastAPI service"
                        },
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
                except Exception as e:
            ...
            if target_node_type == 'filter' and join_node is None:
                ...
                logger.info(f"PipelineQueryExecutionView: executing filter pipeline")
                logger.info(f"  target_node_id={target_node_id}, target_type={target_node_type}")
                ...
                try:
                    ...
                    try:
                        ...
                        if 'output_metadata' in input_node_data and input_node_data['output_metadata']:
                        ...
                        if input_schema and isinstance(input_schema, list):
                            ...
                            logger.warning(f"Filter: No input schema found, columns may lose type information")
            ...
            if target_node_type == 'projection':
                logger.info(f"🔧 CODE VERSION: 2026-01-20-12:05 - Schema Validation Fix Applied")
                logger.info(f"PipelineQueryExecutionView: executing projection pipeline")
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(f"[FETCH DEBUG] ✗ Database query failed (table may not exist): {str(db_error)}")
                        canvas_node = None
                    if canvas_node:
                        ...
                        if canvas_node.config_json:
                            ...
                            if db_calculated:
                                ...
                                logger.info(f"[FETCH DEBUG] ✗ CanvasNode.config_json exists but no calculatedColumns found")
                                logger.info(f"[FETCH DEBUG]   config_json keys: {list(canvas_node.config_json.keys())}")
                        else:
                            logger.info(f"[FETCH DEBUG] ✗ CanvasNode found but config_json is empty or None")
                    else:
                ...
                logger.info(f"[FETCH DEBUG] Merging calculated columns from all sources:")
                ...
                if calculated_columns:
                    logger.info(f"[FETCH DEBUG] Final calculated columns list:")
                    for cc in calculated_columns:
                        logger.info(f"[FETCH DEBUG]   * {cc.get('name')}: {cc.get('expression')}")
                else:
                    logger.warning(f"[FETCH DEBUG] ⚠️  NO CALCULATED COLUMNS DETECTED FROM ANY SOURCE!")
                ...
                logger.info(f"[Calculated Column Debug] Checking all locations for calculated columns:")
                ...
                logger.info(f"[Calculated Column Debug] Checking for 'upper' column in projection_metadata.columns:")
                ...
                if input_node_type == 'source':
                    ...
                    logger.info(f"Projection after join: Executing join first, then applying projection")
                    ...
                    if join_rows is None:
                        ...
                        try:
                            ...
                            logger.error(traceback.format_exc())
                            return Response(
                                {"error": f"Failed to execute join for projection: {str(e)}"},
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR
                            )
                ...
                if password_value is None or (isinstance(password_value, str) and password_value.strip() == ''):
                    ...
                    logger.warning(f"Projection: Source config password type: {type(password_value)}, value: {repr(password_value)}")
                    db_cursor.close()
                    conn.close()
                    return Response(
                        {
                ...
                if not password_str:
                    logger.error(f"Projection: Password became empty after conversion. Original type: {type(password_value)}, value: {repr(password_value)}")
                    db_cursor.close()
                    conn.close()
                    return Response(
                        {
                ...
                if has_aggregates:
                    ...
                    # Fallback: Get projection output metadata if selected_columns is empty (shouldn't happen)
                        ...
                        logger.warning(f"[Projection] ⚠️ FALLBACK: selected_columns is empty, using saved metadata. This may contain stale columns!")
                        ...
                        logger.info(f"[Projection DEBUG] First 3 columns structure:")
                        ...
                        if output_metadata_cols:
                            logger.info(f"[Projection DEBUG] First 3 output_metadata_cols structure:")
                        ...
                        if valid_calculated_columns and len(valid_calculated_columns) > 0:
                            logger.info(f"[Projection] Rebuilding output_metadata with calculated columns at end")
                            ...
                            if not selected_columns or len(selected_columns) == 0:
                                logger.warning(f"[Projection Rebuild] ⚠️ selected_columns is empty! This will cause only calculated columns to show.")
                                logger.warning(f"[Projection Rebuild] Checking projection_columns_metadata for base columns...")
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Database table might not exist - this is OK for preview/execution
                            logger.warning(f"[Calculated Column Fallback] Database query failed (table may not exist): {str(db_error)}")
                            canvas_node = None
                        if canvas_node and canvas_node.output_metadata:
                            output_meta_cols = canvas_node.output_metadata.get('columns', [])
                            if output_meta_cols:
                                logger.info(f"[Calculated Column Fallback] Checking CanvasNode.output_metadata.columns for calculated columns")
                                existing_calc_names = {cc.get('name') for cc in valid_calculated_columns if cc.get('name')}
                                found_new = False
                    ...
                    # ============================================================
                    logger.info(f"[Schema Validation] Fetching actual table schema to validate columns")
                    ...
                    try:
                        ...
                        if schema_response.status_code == 200:
                            ...
                            for col_meta in output_metadata_columns:
                                ...
                                if col_source == 'calculated':
                                    ...
                                    if col_meta.get('expression'):
                                        ...
                                        logger.error(f"[Schema Validation]    This is a FRONTEND BUG - calculated columns must be saved with their expressions")
                                    ...
                                    if col_name.lower() in actual_column_names:
                                        ...
                                        if col_name in calculated_col_names_set:
                                            ...
                                            logger.info(f"[Schema Validation]    Marking as calculated column - will be evaluated in Python")
                                            ...
                                            logger.warning(f"[Schema Validation]    This column will be EXCLUDED from the query")
                                            logger.warning(f"[Schema Validation]    Possible causes:")
                                            logger.warning(f"[Schema Validation]      1. Column was deleted from table")
                                            logger.warning(f"[Schema Validation]      2. This is a calculated column missing its expression (FRONTEND BUG)")
                                            columns_without_expression.append(col_name)
                            ...
                            logger.warning(f"[Schema Validation] Proceeding without validation - may encounter SQL errors")
                ...
                try:
                    ...
                    if not password_str or password_str.strip() == '':
                        logger.error(f"Projection: password_str is empty before building connection_data. password_value type: {type(password_value)}, value: {repr(password_value)}")
                        db_cursor.close()
                        conn.close()
                        return Response(
                            {
                    ...
                    if not connection_data.get('password') or connection_data.get('password') == '':
                        logger.error(f"Projection: Password is missing or empty in connection_data before sending to FastAPI")
                        logger.error(f"Projection: connection_data keys: {list(connection_data.keys())}")
                        logger.error(f"Projection: password_str value: {repr(password_str)}")
                        db_cursor.close()
                        conn.close()
                        return Response(
                            {
                    ...
                    if cached_base_rows is not None:
                        logger.info(f"[Cache] Using cached base rows and reprocessing calculated columns")
                        ...
                        if missing_needed:
                            ...
                            logger.info(f"[Cache] Fetching fresh data from FastAPI to get all columns")
                            cached_base_rows = None  # Reset to force FastAPI call
                        else:
                            # Use cached rows - they have all needed columns
                            logger.info(f"[Cache] Cached rows have all needed columns, using cached data")
                    ...
                    if rows:
                        ...
                        logger.info(f"[SQL DEBUG] ===== COMPLETE FIRST ROW DATA =====")
                        ...
                        logger.info(f"[SQL DEBUG] Checking for supposedly missing columns:")
                        for col in missing_cols_to_check:
                            if col in rows[0]:
                                logger.info(f"[SQL DEBUG]   ✓ '{col}' EXISTS in SQL result, value: {repr(rows[0][col])}")
                            else:
                                # Check case-insensitive
                                found = False
                                for key in rows[0].keys():
                                    if key.lower() == col.lower():
                                        logger.info(f"[SQL DEBUG]   ✓ '{col}' EXISTS (as '{key}'), value: {repr(rows[0][key])}")
                                        found = True
                                        break
                                if not found:
                                    logger.warning(f"[SQL DEBUG]   ✗ '{col}' DOES NOT EXIST in SQL result")
                        
                        logger.info(f"[SQL DEBUG] ===== END COMPLETE ROW DATA =====")
                        ...
                        logger.warning(f"[SQL Execution] No rows returned from FastAPI")
                    ...
                    if output_metadata_columns and rows:
                        logger.info(f"[Calculated Column] Checking output_metadata_columns for late-detected calculated columns")
                    ...
                    if all_calculated_columns:
                        ...
                        logger.info(f"[Calculated Column] No calculated columns found in any location")
                    ...
                    if all_calculated_columns and rows:
                        ...
                        calc_logs_dict = {log['name']: log for log in calculated_column_logs}
                        ...
                        for row_idx, row in enumerate(rows):
                            for calc_col in all_calculated_columns:
                                ...
                                try:
                                    # Use production-ready evaluator
                                    from api.utils.calculated_column_evaluator import evaluate_calculated_expression
                                    ...
                                    if row_idx < 3:
                                        logger.info(f"[Calc Column] Row {row_idx + 1}: {calc_name} = {repr(evaluated_value)}")
                                                ...
                                                if col.lower() in expr_lower:
                                                    ...
                                                    for row_key, row_val in row.items():
                                                        if row_key.lower() == col.lower():
                                                            row_value = row_val
                                                            break
                                                    logger.warning(f"[Calc Column] Input column '{col}' (row key match) = {repr(row_value)}")
                                    ...
                                    logger.error(f"[Calculated Column] Error evaluating {calc_name}: {str(e)}")
                                    import traceback
                                    logger.error(traceback.format_exc())
                                    row[calc_name] = None
                        ...
                        for row_idx, row in enumerate(rows[:3]):
                            ...
                            for calc_col in all_calculated_columns:
                                ...
                                if calc_value is None:
                                    ...
                                    logger.info(f"[Row Eval] {calc_name} = {repr(calc_value)} (type: {type(calc_value).__name__})")
                            ...
                            if projected_rows and projected_columns:
                                ...
                                if calc_cols_in_cache:
                                    ...
                                    if first_row_keys == expected_order:
                                        logger.info(f"[Cache Save] ✓ Column order correct: base columns first, calculated at end")
                                    else:
                    ...
                    # ============================================================
                    logger.info("=" * 80)
                    logger.info(f"[RESPONSE DEBUG] Returning preview data to frontend")
                    ...
                    if projected_rows and calc_cols_in_response:
                        ...
                        logger.info(f"[RESPONSE DEBUG] First row calculated column values:")
                        for cc in calc_cols_in_response:
                            col_name = cc['name']
                            col_value = first_row.get(col_name)
                            logger.info(f"[RESPONSE DEBUG]   * {col_name} = {repr(col_value)}")
                    ...
                    return Response(
                        {"error": f"Failed to execute projection: {str(e)}"},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
            ...
            if target_node_type == 'aggregate':
                logger.info(f"PipelineQueryExecutionView: executing aggregate pipeline")
                logger.info(f"  target_node_id={target_node_id}")
                ...
                if input_node_type == 'source':
                    ...
                    logger.info(f"[Aggregate Debug] Input is PROJECTION node, tracing back to source")
                    current_node = input_node
                    trace_path = [current_node.get('id')]
                    ...
                    while current_node:
                        ...
                        current_node = next((n for n in nodes if n.get('id') == parent_edge.get('source')), None)
                        if not current_node:
                            logger.warning(f"[Aggregate Debug] Parent node not found")
                            break
                        ...
                        if current_type == 'source':
                            ...
                            # Another projection, keep going back
                            logger.info(f"[Aggregate Debug] Found another projection, continuing trace")
                            continue
                        else:
            ...
            # ------------------------------------------------------------------
            if target_node_type == 'compute':
                logger.info(f"PipelineQueryExecutionView: executing compute pipeline")
                logger.info(f"  target_node_id={target_node_id}")
                ...
                try:
                    ...
                    try:
                        ...
                        if requirements:
                            logger.info(f"[Compute] Installing dependencies from requirements.txt")
                            ...
                            try:
                                ...
                                logger.info(f"[Compute] Dependencies installed successfully")
                            finally:
                                # Clean up temp file
                                try:
                        ...
                        local_vars = {
                            '_input_df': _input_df_data,  # Primary input (as per contract)
                            'input_df': _input_df_data,   # Alias for convenience
                            'df': _input_df_data,         # Common alias
                            'pd': pd,
                            '_output_df': None,
                        ...
                        _output_df = local_vars.get('_output_df')
                        if _output_df is None:
                            _output_df = local_vars.get('output_df')
                        
                        if _output_df is None:
                            return Response(
                                {"error": "Code must assign a DataFrame to '_output_df' or 'output_df'"},
                                status=status.HTTP_400_BAD_REQUEST
                            )
                        ...
                        logger.error(f"[Compute] Dependency installation timeout")
                        return Response(
