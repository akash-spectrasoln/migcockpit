...
class MetadataViewSet(viewsets.ViewSet):
    ...
    def validate_pipeline(self, request):
        ...
        if len(source_nodes) == 0:
            ...
            logger.error(f"[VALIDATE] No source nodes found")
        else:
        ...
        if len(dest_nodes) == 0:
            ...
            logger.error(f"[VALIDATE] No destination nodes found")
        else:
        ...
        logger.info(f"[VALIDATE] Step 2: Creating execution plan via Migration Service")
        
        try:
            # Call Migration Service to validate and create execution plan
            logger.debug(f"[VALIDATE] Calling Migration Service API")
            ...
            if result.get("success"):
                logger.info(f"[VALIDATE] ✓✓✓ Validation SUCCESSFUL ✓✓✓")
                logger.info(f"[VALIDATE] Execution plan created and ready for execution")
            ...
            errors.append(f"Failed to create execution plan: {str(e)}")
            return Response({
                "valid": False,
                "errors": errors,
                "warnings": warnings,
