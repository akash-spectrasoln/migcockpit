...
class CheckpointCacheManager:
    ...
    def save_checkpoint(self, node_id: str, node_type: str, node_config: Dict, 
                        upstream_version_hash: str, columns: List[Dict], 
                        sql_query: str = None, sql_params: List = None, 
                        rows: List[Dict] = None):
        """
        ...
        try:
            ...
            if rows is not None:
                ...
                if not col_names:
                    logger.warning(f"[CHECKPOINT SAVE] No columns available, skipping checkpoint")
                    return False
    ...
    def invalidate_downstream(self, node_id: str, nodes: List[Dict], edges: List[Dict]):
        ...
        targets = [node_id] + list(downstream_ids)
