...
def _log_request_metadata(endpoint: str, request_data: Dict[str, Any], connection_config: Dict[str, Any]):
    ...
    def _slice_str(s: str, n: int) -> str:
        if len(s) <= n: return s
        chars: List[str] = []
        for i in range(min(n, len(s))): chars.append(s[i])
        return "".join(chars)
...
async def get_columns(request: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch table columns for {table_name}: {str(e)}"
        )
    ...
    try:
        ...
        async def fetch_table_columns(table_info: Dict[str, Any]):
            ...
            try:
                ...
                logger.warning(f"Failed to fetch columns for table {table_info.get('table_name')}: {str(e)}")
                return {
                    "table_name": table_info.get('table_name'),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch columns in bulk: {str(e)}"
        )
    ...
    try:
        ...
        if source_type not in ["postgresql", "mysql", "oracle", "sqlserver"]:
            return {
                ...
                "error": f"Supported types: postgresql, mysql, oracle, sqlserver"
            }
...
async def execute_filter(request: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to execute filter: {str(e)}"
        )
...
async def execute_aggregate(request: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to execute aggregate: {str(e)}",
        )
...
async def execute_join(request: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to execute join: {str(e)}"
        )
