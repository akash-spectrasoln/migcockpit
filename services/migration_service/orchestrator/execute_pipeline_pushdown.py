...
from orchestrator.ws_emitter import get_ws_emitter
    ...
    try:
        ...
        if execution_plan is None:
            ...
            if not connection_config and config.get("destination_configs"):
                dest_config = list(config["destination_configs"].values())[0]
        ...
        if dest_creates:
            ...
            _log_phase(_last_phase)
            for idx, create_sql in enumerate(dest_creates):
...
def _get_destination_connection(config: Dict[str, Any]):
    ...
    dest_config = list(destination_configs.values())[0]
