...
class FilterPushdownOptimizer:
    ...
    def __init__(self, nodes: Dict[str, Any], edges: List[Dict[str, Any]], config: Dict[str, Any] = None):
        logger.debug(f"[PUSHDOWN] Initializing optimizer...")
        self.nodes = nodes
        self.edges = edges
        ...
        try:
            ...
            logger.debug(f"[PUSHDOWN] Building column lineage...")
            self._build_lineage()
            logger.debug(f"[PUSHDOWN] Lineage built for {len(self.lineage.lineage)} nodes")
            
            logger.debug(f"[PUSHDOWN] Indexing calculated columns...")
    ...
    def _build_lineage(self):
        ...
        if not canvas_id:
            logger.warning(f"[LINEAGE] No canvas_id in config, cannot read from cache")
            return
        ...
        try:
            ...
            if not connection_config:
                logger.warning(f"[LINEAGE] No connection_config, cannot read from cache")
                return
    ...
    def _analyze_simple_column_filter(
        ...
        if filter_node_id not in self.lineage.lineage:
            ...
            return PushdownDecision(
                can_push=False,
                reason=f"No lineage information for filter node"
            )
    ...
    def optimize_all_filters(self) -> Dict[str, Any]:
        ...
        if filter_count == 0:
            logger.info(f"[PUSHDOWN] No filter nodes found in pipeline")
        else:
...
def analyze_filter_pushdown(
    ...
    try:
        ...
        if isinstance(nodes, list):
            logger.debug(f"[PUSHDOWN] Converting nodes from list to dict...")
            nodes_dict = {}
            for node in nodes:
        ...
        logger.debug(f"[PUSHDOWN] Creating FilterPushdownOptimizer...")
        optimizer = FilterPushdownOptimizer(nodes, edges, config)
        logger.debug(f"[PUSHDOWN] Optimizer created successfully")
        
        # Run optimization
        logger.debug(f"[PUSHDOWN] Running optimize_all_filters...")
        ...
        import traceback
        logger.error(f"[PUSHDOWN] Full traceback:")
