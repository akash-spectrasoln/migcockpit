...
def compile_join_sql(
    ...
    join_sql = f'''SELECT {select_clause}
...
def _infer_columns(
    ...
    # 3. Recursive trace
    if reverse_adj is None: reverse_adj = _build_reverse_adjacency(edges)
    parents = reverse_adj.get(node_id, [])
    
    res = None
    if node_type == "join" and len(parents) >= 2:
        l = _infer_columns(parents[0], nodes, edges, config, cache, reverse_adj)
...
def compile_staging_table_sql(
    ...
    def _guard_self_ref(sql: str) -> None:
        ...
        if staging_table in source_part or quoted_staging in source_part:
            raise SQLCompilationError(
                ...
                f"Check traverse_upstream() resolves the correct parent node."
            )
    ...
    if columns:
        ...
        has_prefix = lambda s: "_" in s and (s.startswith("9aad5245") or s.startswith("83654bba") or s.startswith("_L_") or s.startswith("_R_") or len(s) > 10)
...
def flatten_segment(
    ...
    *,
    ...
    final_node = nodes.get(final_id, {})
    final_type = _get_node_type(final_node)
    final_config = final_node.get("data", {}).get("config", {})
    ...
    required_ordered = [name for name in select_expressions.keys()]
