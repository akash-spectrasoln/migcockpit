...
def compute_plan_hash(
    ...
    plan_structure = {
        ...
        "materialization_points": sorted(list(materialization_points.keys())) if materialization_points else []
    }
...
def save_execution_plan_to_db(connection_config: Dict[str, Any], canvas_id: str, plan_hash: str, plan_obj: ExecutionPlan) -> bool:
    ...
    try:
        ...
        if not connection_config:
            logger.error(f"[PLAN PERSIST] No connection_config provided")
            return False
        
        if not canvas_id:
            logger.error(f"[PLAN PERSIST] No canvas_id provided")
            return False
        ...
        logger.info(f"[PLAN PERSIST] Connected successfully, creating schema/table")
        ...
        logger.info(f"[PLAN PERSIST] Serializing plan object")
...
def _build_adjacency(edges: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    for edge in edges:
        ...
        if source not in adjacency: adjacency[source] = []
