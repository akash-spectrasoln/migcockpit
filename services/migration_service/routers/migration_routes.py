...
from orchestrator.execute_pipeline_pushdown import (
...
from state.job_store import get_job, set_job, update_job, list_job_ids
...
async def execute_migration(request: MigrationRequest):
    ...
    try:
        ...
        if execution_plan is not None:
            logger.info(f"[EXECUTE] execution_plan provided in request payload — will use it directly")
            ...
            if cc:
                ...
                logger.warning(f"[EXECUTE] no connection_config in config — will only try destination DBs")
            ...
            if not configs_to_try:
                logger.warning(f"[EXECUTE] no DB config available — cannot load saved plan; will build plan (SQL compilation)")
            ...
            for label, conn_cfg in configs_to_try:
                ...
                if saved is None:
                    ...
                    if stored_hash == current_hash:
                        ...
                        logger.info(f"[EXECUTE]   → hash mismatch — trying next DB or will recompile")
            ...
            logger.warning(f"[EXECUTE] canvas_id is None — skipping plan load")
