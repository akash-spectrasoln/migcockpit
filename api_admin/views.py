...
from api_admin.serializers import CustomerSerializer, CountrySerializer
...
from api_admin.serializers import UserSerializer
...
class CustomerListView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"error": f"Error retrieving customers: {str(e)}"}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class CustomerUserUpdateView(APIView):
    ...
    def get(self, request, id):
        ...
        try:
            ...
            return Response(
                {"error": f"Error retrieving customer: {str(e)}"}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def put(self, request,id):
        ...
        try:
            ...
            return Response(
                {"error": f"Error updating customer and user: {str(e)}"}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
