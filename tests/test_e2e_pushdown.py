...
def run_test():
    ...
    try:
        ...
        ''')
        print(f"    ✓ Created trad_connections (5 rows)")
        ...
        ''')
        print(f"    ✓ Created trad_log_updates (8 rows)")
        ...
        print(f"    Pipeline:")
        print(f"    src_conn → proj_1 → filter_1 ──→ JOIN ──→ proj_3 → dest_1")
        print(f"    src_log  → proj_2 ──────────────→ JOIN")
        ...
        if plan.final_insert_sql:
            ...
            try:
                ...
                print(f"\n    ✗ INSERT failed: {e}")
                print(f"    (This is expected if destination table structure doesn't match)")
            ...
            print(f"    No final INSERT SQL generated")
        ...
        print(f"    ✓ Dropped this job's staging tables")
        ...
        print(f"  ✓ Step 1: Source tables created with sample data")
        print(f"  ✓ Step 2: Pipeline DAG defined ({len(nodes)} nodes, {len(edges)} edges)")
        print(f"  ✓ Step 3: Validation passed, execution plan built")
        print(f"  ✓ Step 4: SQL executed against PostgreSQL")
        print(f"  ✓ Step 5: Staging tables verified ({len(tables)} tables)")
        print(f"  ✓ Step 6: Final INSERT executed")
        print(f"  ✓ Step 7: Destination data verified")
        print(f"  ✓ Step 8: Cleanup completed")
        print()
        print(f"  Plan Hash: {plan_hash[:16]}...")
        print(f"  Materialization Points: {len(mat_points)}")
        print(f"  Staging Tables Created: {len(tables)}")
        print(f"  Total SQL Queries: {plan.total_queries}")
        print(f"  Python Rows Processed: 0  (ALL in PostgreSQL)")
        print()
